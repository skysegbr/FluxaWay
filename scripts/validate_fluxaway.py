#!/usr/bin/env python3
"""Static validation for FluxaWay examples and distributable files.
...
# similar asset references look in FluxaWay's no-JSX call syntax. Deliberately
...
# Per-category FluxaWay UI CSS (mirrors the JS component modules). An example that
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate FluxaWay static files.")
    parser.add_argument(
        "root",
        nargs="?",
        default=".",
    ...
    if issues:
        print("FluxaWay static validation failed:\n")
        for issue in issues:
            print(f"- {issue.path.relative_to(root)}: {issue.message}")
        return 1

    print("FluxaWay static validation passed.")
    return 0
...
def validate_js_asset_srcs(root: Path) -> list[Issue]:
    ...
    HTML's <img src> etc. are already covered by validate_html_file, but FluxaWay
...
def _component_category_map(dist: Path) -> dict[str, str]:
    ...
    for cat in CSS_CATEGORIES:
        path = dist / f"fluxaway-components-{cat}.js"
        if not path.is_file():
            continue
...
def _class_category_map(dist: Path) -> dict[str, set[str]]:
    ...
    for cat in CSS_LOAD_ORDER:
        path = dist / f"fluxaway-ui-{cat}.css"
        if not path.is_file():
            continue
...
def validate_example_css_categories(root: Path) -> list[Issue]:
    """An example that links per-category FluxaWay CSS must load base plus
    ...
    the monolith (fluxaway-ui.css) already contain everything and are skipped."""
    dist = root / "dist"
    examples_dir = root / "examples"
    if not (dist / "fluxaway-ui-base.css").is_file() or not examples_dir.is_dir():
        return []
    ...
    for index in sorted(examples_dir.glob("*/index.html")):
        linked = FLUXAWAY_UI_LINK_RE.findall(index.read_text(encoding="utf-8"))
        ...
        linked_set |= set(FLUXAWAY_UI_RUNTIME_RE.findall(source))
        needed = {"base"}
        for match in FLUXAWAY_COMPONENT_IMPORT_RE.finditer(source):
        ...
        if missing:
            issues.append(
                Issue(
                    ...
                    + ", ".join(f"fluxaway-ui-{c}.css" for c in missing)
                    + " (those components would render unstyled); add the link(s) "
                    "or switch to the fluxaway-ui.css monolith.",
                )
            )
    return issues
