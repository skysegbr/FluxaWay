/*!
 * FluxaWay — core runtime: h(), render(), hooks, context, router, SSR.
 *
...
const PORTAL = Symbol("FluxaWay.Portal");
...
export const Fragment = Symbol("FluxaWay.Fragment");
...
function warnInnerHTMLChildren(children) {
  ...
  if (meaningful) {
    ...
    console.warn(
      "FluxaWay: an element received both `innerHTML` and children — the children were dropped. Give the element only one of them.",
    );
  }
}
...
// FluxaWay runs component functions immediately when you call h(Component, props)
...
export function useErrorBoundary() {
  ...
  function guard(render) {
    ...
    try {
      ...
      console.error("FluxaWay: useErrorBoundary caught a render error — showing fallback UI.", caught);
      setError(caught);
      return null;
    }
  }
...
// FluxaWay never rewrites your URLs automatically — a data: image or a custom app
...
function scheduleRender(root, dirtyOwner) {
  if (!dirtyOwner || dirtyOwner === root) {
    ...
    if (!warnedUnmountedOwners.has(dirtyOwner)) {
      ...
      console.warn(
        "FluxaWay: setState called on an unmounted component — ignored. A stale setter usually means an effect is missing its cleanup (subscription, timer, event listener).",
      );
    }
    return;
  } else {
...
function fullRender(root, carriedEffects) {
  ...
  try {
    ...
    console.error("FluxaWay: render failed — keeping the last successful UI on screen.", error);
  }
}
...
function targetedRender(root, owner, deferredEffects) {
  ...
  try {
    ...
    console.error("FluxaWay: a targeted re-render threw — retrying from the root.", error);
    failed = true;
  } finally {
...
function runEffects(root) {
  ...
  for (const { owner, cursor, effect } of toRun) {
    ...
    if (typeof hook.cleanup === "function") {
      runSafely(hook.cleanup, "FluxaWay: an effect's cleanup threw before re-running.");
    }

    const cleanup = runSafely(effect, "FluxaWay: an effect threw.");
...
function cleanupEffects(root) {
  ...
  for (const hook of root.hooks) {
    if (typeof hook?.cleanup === "function") {
      runSafely(hook.cleanup, "FluxaWay: an effect's cleanup threw during unmount.");
    }
  }
...
/*
 * FluxaWay evaluates h(Child) eagerly — children render before their parent
...
// Sets data-palette on <html>; fluxaway-ui.css pairs each palette with both a
...
// directly as an inline style on <html>, and fluxaway-ui.css derives
...
// ── usePresence ────────────────────────────────────────────
//
// Keeps elements mounted through their exit animation. FluxaWay removes a DOM
...
export function hydrate(component, container) {
  ...
  try {
    ...
    console.error("FluxaWay: hydration failed — falling back to a clean client render.", error);
