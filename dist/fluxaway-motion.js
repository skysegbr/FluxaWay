/*!
 * FluxaWay — Motion add-on (Flash-style timeline animation).
 *
 ...
 */
import { useEffect, useRef } from "./fluxaway.js";
...
function parseColor(value) {
  ...
  throw new Error(`fluxaway-motion: unsupported color "${value}" — use hex or rgb()/rgba().`);
}
...
export function createTimeline(spec = {}) {
  ...
  const resolveTime = (target) => {
    ...
    throw new Error(`fluxaway-motion: unknown label or time "${target}".`);
  };
  ...
  const controller = {
    ...
    // Stable callback ref binding an element to a named track. FluxaWay calls it
      ...
      callback = (node) => {
        ...
        if (node) {
          ...
          if (compiled) {
            ...
            console.warn(
              `fluxaway-motion: track "${name}" is not in this timeline's spec — the element is bound but nothing will animate it.`,
            );
          }
        } else {
